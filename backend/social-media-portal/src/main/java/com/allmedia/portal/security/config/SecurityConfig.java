...
import com.allmedia.portal.security.service.CustomDefaultOAuth2UserService;
...
//					.antMatchers("/api/v1/test/**").hasAuthority("SCOPE_SUPER_USER")
	...
	public SecurityFilterChain securityFilterChainFacebook(HttpSecurity http,
		...
		return http.requestMatchers().antMatchers("/oauth2/authorization/facebook","/login/oauth2/code/facebook")
			...
			.and()
			.oauth2Login(auth->
				auth.successHandler(facebookAuthenticationSuccessHandler)
				.authorizedClientService(oAuth2AuthorizedClientService))
			.build();
	}
	@Bean
