...
import com.allmedia.portal.security.repo.OAuth2AuthorizedClientRepo;
...
public class CustomOAuth2AuthorizedClientService implements OAuth2AuthorizedClientService {
	...
	public void saveAuthorizedClient(OAuth2AuthorizedClient authorizedClient, Authentication principal) {
		MyOAuth2AuthorizedClient oAuth2AuthorizedClient = mapToMyOAuth2AuthorizedClient(authorizedClient,principal);
		authorizedClientRepository.save(oAuth2AuthorizedClient);
	...
	private OAuth2AccessToken mapToToken(MyOAuth2AuthorizedClient auth2AuthorizedClient) {
		...
		OAuth2AccessToken oAuth2AccessToken=new OAuth2AccessToken(
			TokenType.BEARER,
			new String(auth2AuthorizedClient.getAccessTokenValue()),
	...
	private MyOAuth2AuthorizedClient mapToMyOAuth2AuthorizedClient(OAuth2AuthorizedClient authorizedClient,Authentication authentication) {
		MyOAuth2AuthorizedClientId id = MyOAuth2AuthorizedClientId.builder()
			...
			.build();
		return MyOAuth2AuthorizedClient.builder()
			.id(id)
			...
			.facebookPageToken(Set.of())
			.build();
	}
	public void update(MyOAuth2AuthorizedClient auth2AuthorizedClient) {
		authorizedClientRepository.saveAndFlush(auth2AuthorizedClient);
	}
	

	public Set<MyOAuth2AuthorizedClient> findByUserEmail(String email) {
		return authorizedClientRepository.findByUserEmail(email).get();
	}
}
